...
function rowToRealmState(row: RealmRow): RealmState {
  return {
    ...
    budget:
      row.budget_daily_limit != null || row.budget_monthly_limit != null
        ? {
...
export class RealmRepo {
  ...
  getById(id: string): RealmState {
    const row = this.db.prepare("SELECT * FROM realms WHERE id = ?").get(id) as
      | RealmRow
      | undefined;
    if (!row) {
      throw new RealmNotFoundError(id);
    }
    return rowToRealmState(row);
  }

  findByName(name: string): RealmState | null {
    const row = this.db.prepare("SELECT * FROM realms WHERE name = ?").get(name) as
      | RealmRow
      | undefined;
  ...
  update(
    ...
    if (data.name !== undefined) {
      this.db
        .prepare("UPDATE realms SET name = ?, updated_at = ? WHERE id = ?")
        .run(data.name, now, id);
    }
    if (data.description !== undefined) {
      this.db
        .prepare("UPDATE realms SET description = ?, updated_at = ? WHERE id = ?")
        .run(data.description, now, id);
    }
    if (data.status !== undefined) {
      this.db
        .prepare("UPDATE realms SET status = ?, updated_at = ? WHERE id = ?")
        .run(data.status, now, id);
    }
    if (data.icon !== undefined) {
      this.db
        .prepare("UPDATE realms SET icon = ?, updated_at = ? WHERE id = ?")
        .run(data.icon, now, id);
    }
    if (data.proactiveEnabled !== undefined) {
      this.db
        .prepare("UPDATE realms SET proactive_enabled = ?, updated_at = ? WHERE id = ?")
        .run(data.proactiveEnabled ? 1 : 0, now, id);
    }
  ...
  updateActivity(id: string): void {
    const now = new Date().toISOString();
    this.db
      .prepare("UPDATE realms SET last_activity = ?, updated_at = ? WHERE id = ?")
      .run(now, now, id);
  }
  ...
  updateHealthScore(id: string, healthScore: number, riskCount?: number): void {
    ...
    if (riskCount !== undefined) {
      this.db
        .prepare("UPDATE realms SET health_score = ?, risk_count = ?, updated_at = ? WHERE id = ?")
        .run(healthScore, riskCount, now, id);
    } else {
      this.db
        .prepare("UPDATE realms SET health_score = ?, updated_at = ? WHERE id = ?")
        .run(healthScore, now, id);
    }
  }
}
