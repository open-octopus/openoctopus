...
export async function processChatMessage(params: ChatPipelineParams): Promise<ChatPipelineResult> {
  ...
  if (params.entityId) {
    ...
    if (!summoned) {
      throw Object.assign(new Error("Entity must be summoned"), {
        status: 400,
        code: "NOT_SUMMONED",
      });
    }
    ...
    if (intent.action) {
      const actionResult = await handleSystemAction(
        intent.action,
        intent.actionArgs ?? {},
        services,
      );
        ...
        .then((extractionResult) => {
          ...
          if (extractionResult.attributeUpdates.length > 0) {
            for (const update of extractionResult.attributeUpdates) {
              try {
                const foundEntity = services.entityManager.findByNameInRealm(
                  realm.id,
                  update.entityName,
                );
                if (foundEntity) {
  ...
  if (realm && services.maturityScanner) {
    services.maturityScanner
      .checkAndNotify(
        ...
        (progress) => {
          services.wsBroadcaster?.broadcast(createRpcEvent(RPC_EVENTS.MATURITY_PROGRESS, progress));
        },
      )
      .catch(() => {});
  }
...
async function handleSystemAction(
  ...
  switch (action) {
    case "summon": {
      ...
      if (!entityName) {
        return "Please specify which entity to summon.";
      }
    ...
    case "unsummon": {
      const entityName = args.entityName;
      if (!entityName) {
        return "Please specify which entity to unsummon.";
      }
    ...
    case "list_realms": {
      const realms = services.realmManager.list();
      if (realms.length === 0) {
        return "No realms configured yet.";
      }
    ...
    case "list_entities": {
      ...
      if (lines.length === 0) {
        return "No entities found across any realm.";
      }
    ...
    case "switch_realm": {
      ...
      if (!realmName) {
        return "Please specify which realm to switch to.";
      }
