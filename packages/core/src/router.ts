      ...
      try {
        ...
        if (intent) {
          ...
          if (context?.previousRealmId && intent.confidence < 0.5 && !intent.targetRealmId) {
            ...
            if (prevRealm) {
              log.info(
                `Applying realm momentum to "${prevRealm.name}" (previous realm, LLM uncertain)`,
              );
              return {
                ...intent,
        ...
        log.warn(
          `LLM routing failed, falling back to keywords: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
...
- Greetings and small talk should return null unless they mention a specific domain topic`;
    ...
    log.info(
      `LLM routed to "${parsed.realmName}" (confidence=${parsed.confidence.toFixed(2)}, reason=${parsed.reasoning})`,
    );
    ...
    const matchedRealm = realms.find(
      (r) => r.name.toLowerCase() === parsed.realmName!.toLowerCase(),
    );
    if (!matchedRealm) {
      return null;
    }
  ...
  private parseRouterResponse(
    ...
    try {
      ...
      return {
        ...
        confidence:
          typeof data.confidence === "number" ? Math.min(Math.max(data.confidence, 0), 1) : 0.5,
  ...
  routeWithKeywords(message: string, realms: RealmState[], context?: RouteContext): RouterIntent {
    ...
    if (bestMatch) {
      ...
      const maxKeywords = Math.max(
        ...realms.map((r) => (REALM_KEYWORDS[r.name.toLowerCase()] ?? []).length),
        1,
      );
      const confidence = Math.min(bestMatch.score / Math.min(maxKeywords * 0.3, 5), 1.0);
      log.info(
        `Keyword-routed to realm ${bestMatch.realmId} (confidence=${confidence.toFixed(2)})`,
      );
      return {
