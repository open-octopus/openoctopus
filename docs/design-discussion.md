...
<p align="center">
  <img src="images/信息图/八爪鱼人生治理系统.png" alt="八爪鱼人生治理系统" width="720">
</p>
...
| 决策 | 结论 | 理由 |
|---|---|---|
| 域命名 | **Realm**（域） | 自治领地语义，对应章鱼触手独立神经中枢 |
| 实体化命名 | **Summon**（召唤） | "从深海召唤"是章鱼最经典意象，零解释成本，中文"召唤"自带正向联想 |
...
| 核心价值 | 自动化执行 | **人生知识库** + **Summon（召唤）** 代理 |

四个核心设计点：

1. **域划分（Realm）**：人生按域分类，知识分域管理
2. **域内对话**：先选域再对话，沉淀知识到域，同时参考跨域信息
3. **技能分层**：全局 Skill + 域 Skill，Agent 归属于域
4. **召唤（Summon）**：将生活对象（宠物、家人、资产）召唤为有记忆、有性格、可主动行动的 Agent
...
| Summoned Agent | 域内 | 由实体召唤而来，拥有实体的记忆和性格 |
...
将现实世界的对象（人、宠物、资产、组织……）**召唤**为具有**记忆、性格、主动性**的 AI Agent。

```
原始数据 → 结构化实体 → Summon（召唤） → 可交互、可主动行动的 Agent
```
...
> **OpenOctopus — Organize life by realms. Summon anything into a living agent.**
>
> 把人生分域治理，把万物召唤为 Agent，八臂并行，一脑协同。

两大品牌关键词：**Realm**（分域治理）+ **Summon**（万物召唤）——这是 OpenOctopus 独有的，OpenClaw 没有的。
